...
'/root/nsd2020/nsd2009/py02/day01'
>>> os.mkdir('/tmp/nsd2009')  # mkdir /tmp/nsd2009
>>> os.makedirs('/tmp/demo/nsd2009')  # mkdir -p /tmp/demo/nsd2009
>>> os.listdir()  # ls
...
>>> os.rmdir('/tmp/demo/nsd2009')  # rmdir /tmp/demo/nsd2009  只能删空目录
>>> shutil.rmtree('/tmp/nsd2009')
...
>>> os.path.basename('/tmp/demo/nsd2009/a.txt')  # 返回最右面/右侧内容
'a.txt'
>>> os.path.dirname('/tmp/demo/nsd2009/a.txt')   # 返回最右面/左侧内容
'/tmp/demo/nsd2009'
>>> os.path.split('/tmp/demo/nsd2009/a.txt')  # 返回元组
('/tmp/demo/nsd2009', 'a.txt')
>>> os.path.join('/tmp/demo/nsd2009', 'a.txt')  # 路径拼接
'/tmp/demo/nsd2009/a.txt'
```
...
>>> for path, folders, files in os.walk('/tmp/mydemo'):
...   print('%s:' % path)
...   for folder in folders:  # 目录是列表，继续遍历
...     print('\033[34;1m%s\033[0m' % folder, end='\t')
...   for file in files:  # 文件是列表，继续遍历
...     print(file, end='\t')
...   print('\n')
```

复杂数据结构取值：字典都可以通过key取出val，序列对象都可以通过下标取出对应的值

```python
>>> import pprint
>>> l1 = [{'class': 1, 'students': ('zhangsan', 'lisi')}, {'class': 2, 'students': ('tom', 'jerry')}]
>>> pprint.pprint(l1)
[{'class': 1, 'students': ('zhangsan', 'lisi')},
 {'class': 2, 'students': ('tom', 'jerry')}]
>>> l1[0]
{'class': 1, 'students': ('zhangsan', 'lisi')}
>>> d1 = l1[0]
>>> d1['class']
...
>>> data
[['2020-7-21', 0, 0, 10000, 'init data']]
>>> record = ['2020-7-21', 0, 100, 9900, 'buy tickets']
>>> data.append(record)
>>> data
